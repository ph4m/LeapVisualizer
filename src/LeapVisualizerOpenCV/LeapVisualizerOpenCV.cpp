...
CvSize cv_szLeftImage  = cvSize(FORMAT_LEAP_IMAGE_WIDTH,FORMAT_LEAP_IMAGE_HEIGHT),
       cv_szRightImage = cvSize(FORMAT_LEAP_IMAGE_WIDTH,FORMAT_LEAP_IMAGE_HEIGHT);
// Distortion correction
IplImage *cv_leftImageUndistorted  = NULL,
         *cv_rightImageUndistorted = NULL;
...
void SampleListener::onFrame(const Controller& controller) {
    ...
    for(int iCamera = 0; iCamera < 2; iCamera++){
        Image image = images[iCamera];

        const unsigned char* image_buffer = image.data();

        // Check if acquired images have expected dimensions
        if (image.width() != FORMAT_LEAP_IMAGE_WIDTH || image.height() != FORMAT_LEAP_IMAGE_HEIGHT) {
            if (image.width() == 0 && image.height() == 0) {
                std::cout << "No image acquired: "
                          << "width=" << image.width() << "; "
                          << "height=" << image.height() << std::endl;
            } else if (image.width() > FORMAT_LEAP_IMAGE_WIDTH || image.height() > FORMAT_LEAP_IMAGE_HEIGHT) {
                std::cout << "Image dimensions exceed expected dimensions: "
                          << "width=" << image.width() << "; "
                          << "height=" << image.height() << std::endl;
                exit(0);
            } else {
                std::cout << "Image dimensions mismatch: "
                          << "width=" << image.width() << "; "
                          << "height=" << image.height() << std::endl;
                exit(0);
            }
        ...
        for (int row=0; row<image.height(); row++) {
            for (int col=0; col<image.width(); col++) {
                if (iCamera == 0) cvSet2D(cv_leftImage,row,col,cvScalar(image_buffer[cursor]));
                else if (iCamera == 1) cvSet2D(cv_rightImage,row,col,cvScalar(image_buffer[cursor]));
                cursor++;
            }
        }
...
int main(int argc, char** argv) {
    ...
    // Left image
    cv_leftImage=cvCreateImage(cv_szLeftImage,IPL_DEPTH_8U,1);
    if (cv_leftImage==NULL)
    {
        printf("Unable to create left image buffer\n");
        exit(0);
    }
    // Right image
    cv_rightImage=cvCreateImage(cv_szRightImage,IPL_DEPTH_8U,1);
    if (cv_rightImage==NULL)
    {
        printf("Unable to create right image buffer\n");
        exit(0);
    }
    if (flagUndistort) {
        // Left image
        cv_leftImageUndistorted=cvCreateImage(cv_szLeftImageUndistorted,IPL_DEPTH_8U,1);
        if (cv_leftImageUndistorted==NULL)
        {
            printf("Unable to create undistorted left image buffer\n");
            exit(0);
        }
        // Right image
        cv_rightImageUndistorted=cvCreateImage(cv_szRightImageUndistorted,IPL_DEPTH_8U,1);
        if (cv_rightImageUndistorted==NULL)
        {
            printf("Unable to create undistorted right image buffer\n");
            exit(0);
        }
    }
